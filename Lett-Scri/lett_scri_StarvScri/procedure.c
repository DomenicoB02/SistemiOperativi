...
void InizioLettura(int sem, Buffer* buf){
    Wait_Sem(sem, MUTEXL); //inizio a leggere
    // numlettori in mutua esclusione
    buf->numlettori = buf->numlettori + 1;
    
    if (buf->numlettori == 1) //se primo lettore blocca gli scrittori
        Wait_Sem(sem, SYNCH);
...
void FineLettura(int sem, Buffer* buf){
    Wait_Sem(sem, MUTEXL); //sto terminando la lettura, decremento
    // numlettori in mutua esclusione
    buf->numlettori = buf->numlettori - 1;
    
    if (buf->numlettori == 0) //se l'ultimo lettore devo rilasciare la risorsa 
...
void Lettore (int sem, Buffer* buf) {
        ...
        /*********Lettura********/
         ...
         printf("messaggio <%ld>, lettore n.ro %d\n",buf->messaggio,buf->numlettori);
        FineLettura(sem, buf);
}