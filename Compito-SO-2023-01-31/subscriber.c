...
void subscriber(int id_coda_registrazioni, long topic) {
    ...
    */
    ...
    int ret=msgsnd(id_coda_registrazioni,&registrazione,sizeof(messaggio_registrazione)-sizeof(long),0);
    if(ret<0){
    ...
    for(int i=0; i<NUM_MESSAGGI*NUM_PUBLISHERS_PER_TOPIC; i++) {
        ...
        int ret2=msgrcv(id_coda_messaggi_subscriber,&messaggio,sizeof(messaggio_valore)-sizeof(long),topic,0);
        if(ret2<0){
